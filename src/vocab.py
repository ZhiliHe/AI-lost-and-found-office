...
# Whisper picks a script per utterance, and for Mandarin it often writes
# traditional characters no matter which the speaker uses. Our tables are
# simplified, so "我的雨傘在哪裡" would find nothing at all - the demo would
# simply stop understanding Chinese mid-sentence, for a reason invisible on
# screen.
#
# Only the characters our own vocabulary depends on are listed: generated by
# walking the CJK block and keeping every traditional form whose simplified
# equivalent we actually use. It is a substitution, not a translation - one
# character in, one character out - so positions in the text are unchanged and
# the rest of a sentence can still be handed back with its spacing intact.
TRADITIONAL_TO_SIMPLE = {
    "佈": "布", "來": "来", "個": "个", "傘": "伞", "備": "备", "內": "内",
    "剋": "克", "動": "动", "嗎": "吗", "圍": "围", "寶": "宝", "對": "对",
    "屬": "属", "張": "张", "彔": "录", "後": "后", "憑": "凭", "應": "应",
    "捱": "挨", "擋": "挡", "據": "据", "攝": "摄", "數": "数", "書": "书",
    "會": "会", "東": "东", "樓": "楼", "標": "标", "機": "机", "沒": "没",
    "準": "准", "溫": "温", "溼": "湿", "濕": "湿", "為": "为", "爲": "为",
    "發": "发", "盃": "杯", "確": "确", "筆": "笔", "箇": "个", "範": "范",
    "紅": "红", "納": "纳", "紙": "纸", "綠": "绿", "綫": "线", "緊": "紧",
    "緑": "绿", "線": "线", "繩": "绳", "羅": "罗", "聽": "听", "腦": "脑",
    "藍": "蓝", "衕": "同", "裏": "里", "裡": "里", "覈": "核", "記": "记",
    "話": "话", "該": "该", "說": "说", "説": "说", "請": "请", "議": "议",
    "質": "质", "這": "这", "過": "过", "適": "适", "還": "还", "邊": "边",
    "釦": "扣", "鈅": "钥", "鉛": "铅", "銀": "银", "錄": "录", "錢": "钱",
    "録": "录", "錶": "表", "鏡": "镜", "鑰": "钥", "間": "间", "闆": "板",
    "電": "电", "頭": "头", "顏": "颜", "顔": "颜", "風": "风", "馬": "马",
    "髮": "发", "麪": "面", "麫": "面", "麵": "面", "麼": "么", "麽": "么",
    "黃": "黄", "齣": "出",
}


def to_simplified(text):
    """Traditional Chinese in, simplified out. Everything else untouched."""
    raw = str(text)
    if not any(ch in TRADITIONAL_TO_SIMPLE for ch in raw):
        return raw
    return "".join(TRADITIONAL_TO_SIMPLE.get(ch, ch) for ch in raw)
...
def _scan_cjk(text, table):
    ...
    """
    # One substitution pass first, so a Chinese sentence Whisper decided to
    # write in traditional characters still finds our simplified entries. It is
    # character-for-character, so every position below still lines up with the
    # text the caller passed in.
    raw = to_simplified(text)
